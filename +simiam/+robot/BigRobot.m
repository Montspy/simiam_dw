classdef BigRobot < simiam.robot.Robot
...
% see the LICENSE file included with this software
    ...
    methods
        function obj = BigRobot(parent, pose)
           obj = obj@simiam.robot.Robot(parent, pose);
           
           % Add surfaces: BigRobot in top-down 2D view
           ...
           k3_base =  [ -0.025   0.063    1;
            ...
            obj.ir_array(1) = ProximitySensor(parent, 'IR', pose, ir_pose, 0.4, 3, Pose2D.deg2rad(20), 'simiam.robot.BigRobot.ir_distance_to_raw', noise_model);
            
            ir_pose = Pose2D(0.017, 0.063, Pose2D.deg2rad(75));
            obj.ir_array(2) = ProximitySensor(parent, 'IR', pose, ir_pose, 0.4, 3, Pose2D.deg2rad(20), 'simiam.robot.BigRobot.ir_distance_to_raw', noise_model);
            
            ir_pose = Pose2D(0.051, 0.045, Pose2D.deg2rad(42));
            obj.ir_array(3) = ProximitySensor(parent, 'IR', pose, ir_pose, 0.4, 3, Pose2D.deg2rad(20), 'simiam.robot.BigRobot.ir_distance_to_raw', noise_model);
            
            ir_pose = Pose2D(0.067, 0.015, Pose2D.deg2rad(13));
            obj.ir_array(4) = ProximitySensor(parent, 'IR', pose, ir_pose, 0.4, 3, Pose2D.deg2rad(20), 'simiam.robot.BigRobot.ir_distance_to_raw', noise_model);
            
            ir_pose = Pose2D(0.067, -0.015, Pose2D.deg2rad(-13));
            obj.ir_array(5) = ProximitySensor(parent, 'IR', pose, ir_pose, 0.4, 3, Pose2D.deg2rad(20), 'simiam.robot.BigRobot.ir_distance_to_raw', noise_model);
            
            ir_pose = Pose2D(0.051, -0.045, Pose2D.deg2rad(-42));
            obj.ir_array(6) = ProximitySensor(parent, 'IR', pose, ir_pose, 0.4, 3, Pose2D.deg2rad(20), 'simiam.robot.BigRobot.ir_distance_to_raw', noise_model);
            
            ir_pose = Pose2D(0.017, -0.063, Pose2D.deg2rad(-75));
            obj.ir_array(7) = ProximitySensor(parent, 'IR', pose, ir_pose, 0.4, 3, Pose2D.deg2rad(20), 'simiam.robot.BigRobot.ir_distance_to_raw', noise_model);
            
            ir_pose = Pose2D(-0.038, -0.049, Pose2D.deg2rad(-128));
            obj.ir_array(8) = ProximitySensor(parent, 'IR', pose, ir_pose, 0.4, 3, Pose2D.deg2rad(20), 'simiam.robot.BigRobot.ir_distance_to_raw', noise_model);
            
            ir_pose = Pose2D(-0.052, 0.000, Pose2D.deg2rad(180));
            obj.ir_array(9) = ProximitySensor(parent, 'IR', pose, ir_pose, 0.4, 3, Pose2D.deg2rad(20), 'simiam.robot.BigRobot.ir_distance_to_raw', noise_model);
...
%             ir_distances = (0.02-log(ir_array_values/3960)/30)*3/.2;
            ir_distances = ir_array_values;
        end
        ...
        function [vel_r, vel_l] = get_wheel_speeds(obj)
            ...
            vel_r = obj.right_wheel_speed/(R/sf);
            vel_l = obj.left_wheel_speed/(R/sf);
        end
