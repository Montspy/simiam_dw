...
% Dynamic Window style
    ...
    properties (Constant)
        ...
        vDotMax = .5;         % m/s^2
        wDotMax = 60/180*pi;  % rad/s^2
        
        nVSteps = 10;       % Discretize Vd into nVSteps segments along the v axis
        nWSteps = 15;       % Discretize Vd into nWSteps segments along the w axis
...
%                 set(obj.s_net, 'ZData', ones(1,9));
            ...
            if(obj.step == 1) 
                ...
                % 2. Dynamic window (Vd)
                obj.generate_dynamic_window_matrix(obj.nDt*dt);
        ...
        function update_obstacle_line_list(obj, robot, ir_distances, ir_distances_wf)
            ...
            for n=obj.sensorsInd
                ...
                length = max(ir_distances(n) * spreading_multiplier, max_range/6*spreading_multiplier);
...
%             end
        ...
        function compute_admissibility(obj, robot, loopTime)
            ...
            for nV=1:obj.nVSteps
                ...
                for nW=1:obj.nWSteps
                    ...
                    if(sub2ind(size(obj.dist'), nW, nV) < sub2ind(size(obj.dist'), startNW, startNV))
                    ...
                    if obj.Vr(nV, nW) == 1
                        ...
                        if(w_i ~= 0)
                            ...
                            % Min distance
                            maxDist = (obj.vMax*obj.vMax)/obj.vDotMax;  % 2 times the maximum braking distance (from full speed)
    ...
    %                                             set(P, 'ZData', ones(1,100));
        ...
        function [v, w] = maximize_objective_function(obj, dt)
            ...
            figure(2);
            mesh(linspace(obj.wRange(1), obj.wRange(2), obj.nWSteps), ...
...
%             m = 0;      % Min
            ...
            % Normalize dist
            M = 2;%(obj.vMax*obj.vMax)/obj.vDotMax;    % Max
            m = 0;                                  % Min
